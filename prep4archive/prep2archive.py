...
class FileBase:
    ...
    def rename(self):
        ...
        if m:
            ...
            length = (end - start)
            bn = bn[:start] + ", " + \
                m.group(1) + m.group(2) + " Edition" + bn[start+length:]
    ...
    def unduplicate(namelist):
        ...
        try:
            ...
            for t in [(splitext(doc)) for doc in namelist]:
                ...
                documents.append(
                    Document(**(dict(zip(['basename', 'ext'], list(t))))))
        except Exception as e:
            print(e)

        def keyfunc(doc): return doc.basename
        ...
        for key in doc_map.keys():
            if len(doc_map[key]) == 1:
                ...
                def extfunc(ext): return FileBase.ext_order.get(ext, 999)
...
class RAR_or_ZIPFile(FileBase):
    def __init__(self, filename, out_dir):
        ...
        if ext.lower() == ".rar":
            ...
            logging.error(
                "Unsupported type of compressed file: {0}!".format(ext))
    ...
    def process_file(self):
        ...
        # First empty TMPDIR
        files_to_clean = [f for f in listdir(
            TMPDIR) if isfile(join(TMPDIR, f))]
        ...
        extracted_files = [f for f in listdir(
            TMPDIR) if isfile(join(TMPDIR, f))]
        ...
        if len(extracted_files) == 0:
            ...
            # Kludge to deal with zip files that have been processed earlier
            if False:
                zipname = join(self.__out_dir,  titlecase(
                    splitext(basename(self.filename))[0].replace("-", " ")) + ".zip")
            else:
                zipname = join(self.__out_dir,  titlecase(
                    splitext(basename(self.filename))[0]) + ".zip")
            try:
